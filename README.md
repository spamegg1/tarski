...
Current version is 0.2.3 (Mar 10, 2026). Released for Scala 3 only.
...
```scala
//> using dep io.github.spamegg1::tarski:0.2.3
```

For SBT, add to your `build.sbt`:

```scala
libraryDependencies += "io.github.spamegg1" %% "tarski" % "0.2.3"
```
...
Just `:dep io.github.spamegg1::tarski:0.2.3` then `tarski.main.Example.runExample`.
