...
case class Game(
    ...
    commitment: Boolean,
    formula: FOLFormula,
    leftOpt: Option[FOLFormula],
    rightOpt: Option[FOLFormula],
    selected: Option[(block: Block, name: Name)],
    messages: List[Message]
):
  def next(using nm: NameMap): Game = formula match
    case a: FOLAtom =>
      val result  = Interpreter.eval(a)
      val winLose = if commitment == result then "You win!" else "You lose."
      val msg     = s"$winLose $a is $result in this world."
      copy(messages = msg :: messages)
    case And(a, b) =>
      // if commitment then
      //   val msg1 = s"You believe both $a and $b are $commitment"
      //   val msg2 = s"I will try to choose a ${!commitment} formula"
      //   val resA   = Interpreter.eval(a)
      //   val chosen = if resA then b else a
      //   val msg3 = s"I chose $chosen as ${!commitment}"
