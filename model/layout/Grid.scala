...
object Grid:
  extension (grid: Grid)
    /** Converts given `grid` to a [[Board]] by generating fake names for unlabeled [[Block]]s.
      *
      * @return
    ...
    /** Converts given `grid` to a [[Board]] by generating fake names for unlabeled [[Block]]s and adding these labels
