...
class WorldTest extends munit.FunSuite:
  ...
  test("Empty world with no blocks or grid and all names available"):
    assert(w0.panel.isEmpty, s"empty world should have no blocks, but has ${w0.panel}")
    assert(w0.board.isEmpty, s"empty world should have no grid, but has ${w0.board}")
    assert(
  ...
  test("Adding a name to a block, in wrong / correct position"):
    ...
    assertEquals(w9.panel("b").block, b0n, "adding name to block should change its label correctly, but does not")
    assert(
  ...
  test("Removing a name in a world with 2 blocks"):
    assertEquals(w16.board((3, 4)).block, b0, "removing name from named block should work correctly, but does not")
    assertEquals(w16.board((5, 6)).block, b1, "removing name from named block should work correctly, but does not")
    assertEquals(
      w16.panel.values.toSeq,
      Seq((b0, (3, 4)), (b1, (5, 6))),
    ...
    assert(
      w16.board(3, 4).block.label.isEmpty,
